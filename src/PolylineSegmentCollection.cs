...
namespace Sharper.GstarCAD.Extensions.Geometry
{
    ...
    public class PolylineSegmentCollection : List<PolylineSegment>
    {
        ...
        /// Creates a new instance of PolylineSegmentCollection.
        /// </summary>
        /// <param name="polyline">An instance of Polyline.</param>
        public PolylineSegmentCollection(Polyline polyline)
        {
            ...
            {
                Add(new PolylineSegment(
                    polyline.GetPoint2dAt(n),
                    polyline.GetPoint2dAt(0),
                    polyline.GetBulgeAt(n),
                    polyline.GetStartWidthAt(n),
                    polyline.GetEndWidthAt(n)));
            }
        }
        ...
        /// Creates a new instance of PolylineSegmentCollection.
        /// </summary>
        /// <param name="polyline">An instance of Polyline2d.</param>
        public PolylineSegmentCollection(Polyline2d polyline)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                Add(new PolylineSegment(
                    vertex.Position.ToPoint2d(),
                    vertices[i + 1].Position.ToPoint2d(),
                    vertex.Bulge,
                    vertex.StartWidth,
                    vertex.EndWidth));
            }
            ...
            Vertex2d lastVertex = vertices[n];
            Add(new PolylineSegment(
                lastVertex.Position.ToPoint2d(),
                vertices[0].Position.ToPoint2d(),
                lastVertex.Bulge,
        ...
        public PolylineSegmentCollection(Ellipse ellipse)
        {
            ...
            Point3d pt3d2 = ellipse.GetPointAtParameter(Math.PI / 4.0);
            ...
            if (startParam != 0.0)
            {
                ...
                this[startIndex].Bulge *= (1.0 - startParam);
            }

            if (Math.Abs(endParam - 1.0) > 1e-9) //(endParam != 0.0)
            {
                this[endIndex].EndPoint = endPoint;
                this[endIndex].Bulge *= endParam;
            }
        ...
        /// <summary>
        /// Adds the polyline segments of <paramref name="polyline"/> to the current collection.
        /// </summary>
        /// <param name="polyline">An instance of Polyline.</param>
        public void AddRange(Polyline polyline) =>
            AddRange(new PolylineSegmentCollection(polyline));

        /// <summary>
        /// Adds the polyline segments of all polylines in <paramref name="polylines"/> to the current collection.
        /// </summary>
        /// <param name="polylines">A collection of instances of Polyline.</param>
        public void AddRange(IEnumerable<Polyline> polylines) =>
            AddRange(polylines.SelectMany(pl => new PolylineSegmentCollection(pl)));

        /// <summary>
        /// Gets the index of the closest segment to the specified point.
        /// </summary>
        /// <param name="point">The 2d point from which the distances are calculated</param>
        /// <returns>The index of the segment in the collection.</returns>
        public int GetClosestSegmentIndexTo(Point2d point)
        {
            int result = 0;
            double dist = this[0].ToCurve2d().GetDistanceTo(point);
            for (int i = 1; i < Count; i++)
            {
                double tmpDist = this[i].ToCurve2d().GetDistanceTo(point);
                if (!(tmpDist < dist))
                    continue;
        ...
        /// </summary>
        /// <param name="tolerance">The tolerance to be used in points equality comparison.</param>
        /// <returns>A list of instances of PolylineSegmentCollection.</returns>
        public List<PolylineSegmentCollection> Join(Tolerance tolerance)
        {
            ...
            while (clone.Count > 0)
            {
                ...
                while (true)
                {
                    int i = clone.FindIndex(s => s.StartPoint.IsEqualTo(end, tolerance));
                    if (i >= 0)
                    {
                        seg = clone[i];
                        newCol.Add(seg);
                    ...
                    i = clone.FindIndex(s => s.EndPoint.IsEqualTo(end, tolerance));
                    if (i >= 0)
                    {
                        seg = clone[i];
                        seg.Inverse();
                    ...
                    i = clone.FindIndex(s => s.EndPoint.IsEqualTo(start, tolerance));
                    if (i >= 0)
                    {
                        seg = clone[i];
                        newCol.Insert(0, seg);
                    ...
                    i = clone.FindIndex(s => s.StartPoint.IsEqualTo(start, tolerance));
                    if (i >= 0)
                    {
                        seg = clone[i];
                        seg.Inverse();
        ...
        public Polyline ToPolyline()
        {
            ...
            for (int i = 0; i < Count; i++)
            {
                ...
                polyline.AddVertexAt(i, seg.StartPoint, seg.Bulge, seg.StartWidth, seg.EndWidth);
            }

            int j = Count;
            polyline.AddVertexAt(j, this[j - 1].EndPoint, 0.0, this[j - 1].EndWidth, this[0].StartWidth);
            if (!polyline.GetPoint2dAt(0).IsEqualTo(polyline.GetPoint2dAt(j)))
                return polyline;

            polyline.RemoveVertexAt(j);
